...
<body>
			...
			<c:if test="${!empty List }">
				<c:forEach items="${List }" var="dto">
					<tr class="user-info-items">
						...
						<td class="cont"><a
							href="<%=request.getContextPath() %>/member_cont.do?num=${dto.getMember_num() }">
	...
	<nav>
		<ul class="pagination">
			...
			<c:if test="${page == 1 }">
					<a href="member_search.do?page=${startBlock }&search_field=${search_field }&search_keyword=${search_keyword }"
						aria-label="Previous">
				</c:if> 
				
				<c:if test="${page != 1 }">
					<a href="member_search.do?page=${startBlock -1 }&search_field=${search_field }&search_keyword=${search_keyword }"
						aria-label="Previous">
				</c:if>
			...
			<li><c:forEach begin="${startBlock }" end="${endBlock }" var="i">
					<c:if test="${i == page }">
						<a href="member_search.do?page=${i }&search_field=${search_field }&search_keyword=${search_keyword }">${i }</a>
					</c:if>
			...
			<c:if test="${endBlock == allPage }">
				<a href="member_search.do?page=${endBlock }&search_field=${search_field }&search_keyword=${search_keyword }"
					aria-label="Next">
			</c:if> 
				
			<c:if test="${endBlock != allPage }">
				<a href="member_search.do?page=${endBlock +1 }&search_field=${search_field }&search_keyword=${search_keyword }"
					aria-label="Next">
