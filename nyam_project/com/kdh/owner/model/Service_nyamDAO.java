...
public class Service_nyamDAO {
	...
	//싱글턴 방식
	//instance : 참조변수(주소값)
	private static Service_nyamDAO instance;
	
	public Service_nyamDAO() {
	}

	//기본생성자 대신 싱글턴 객체를 return해주는 getInstance()라는 메소드를 만들어서 그 메소드에 외부에서 접근할수 있도록 함
	//어디서 생성하든 항상 동일한 주소값
	public static Service_nyamDAO getInstance() {
		if (instance == null) {
			instance = new Service_nyamDAO();
		}
		return instance;
	}	//getInstance() 끝
	...
	public List<Service_nyamDTO> getServiceList(int page, int rowsize, String id) {
		List<Service_nyamDTO> list = new ArrayList<Service_nyamDTO>();
	...
	//고객센터에 문의글 작성
	public int insertService(Service_nyamDTO dto) {
	...
	//글 내용 가져오는 메소드
	public Service_nyamDTO getCont(int no) {
		Service_nyamDTO dto = new Service_nyamDTO();
	...
	public List<Service_nyamDTO> searchBoardList(String field, String keyword, int page, int rowsize) {
		List<Service_nyamDTO> list = new ArrayList<Service_nyamDTO>();
