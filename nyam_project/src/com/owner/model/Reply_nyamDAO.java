...
public class Reply_nyamDAO {
	...
	//싱글턴 방식
	//instance : 참조변수(주소값)
	private static Reply_nyamDAO instance;
	
	public Reply_nyamDAO() {
	}

	//기본생성자 대신 싱글턴 객체를 return해주는 getInstance()라는 메소드를 만들어서 그 메소드에 외부에서 접근할수 있도록 함
	//어디서 생성하든 항상 동일한 주소값
	public static Reply_nyamDAO getInstance() {
		if (instance == null) {
			instance = new Reply_nyamDAO();
		}
		return instance;
	}	//getInstance() 끝
	...
	public List<Reply_nyamDTO> getReplyList(int page, int rowsize, int no) {
		List<Reply_nyamDTO> list = new ArrayList<Reply_nyamDTO>();
		...
		try {
			...
			while (rs.next()) {
				Reply_nyamDTO dto = new Reply_nyamDTO();
	...
	public List<Reply_nyamDTO> getReportedReplyList(int page, int rowsize, String id) {
		List<Reply_nyamDTO> list = new ArrayList<Reply_nyamDTO>();
	...
	//내가 쓴 댓글 개수 확인하는 메소드
		...
		public List<Reply_nyamDTO> getMyReplyList(int page, int rowsize, String id) {
			List<Reply_nyamDTO> list = new ArrayList<Reply_nyamDTO>();
